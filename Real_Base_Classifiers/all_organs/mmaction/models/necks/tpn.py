...
class SpatialModulation(nn.Module):
    ...
    def forward(self, x): # 对骨干网络中的最后两层特征进行空间语义对齐 [bs, 2048, 8, 8, 8]
        out = []
...
class TemporalModulation(nn.Module):
    ...
    def forward(self, x): # 时间维度的调制 输出都是[bs, 1024, 1, 8, 8]
        x = self.conv(x)
        x = self.pool(x)
        return x
...
class TPN(nn.Module):
    ...
    def forward(self, x, target=None):
        ...
        # Auxiliary loss
        if self.aux_head is not None:
            loss_aux = self.aux_head(x[-2], target) # use res4 output in 3d-resnet 50
        ...
        # Get top-down outs (Top-down flow)
