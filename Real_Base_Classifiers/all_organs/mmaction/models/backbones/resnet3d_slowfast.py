...
class ResNet3dSlowFast(nn.Module):
    ...
    def forward(self, x):
        ...
        Args:
            ...
            t_slow: t/resample_rate(8).
            t_fast: t/(resample_rate(8)/frame_rate_ratio(8)) [default: t]
        ...
        x_slow = nn.functional.interpolate(
            x,
            mode='nearest',
            scale_factor=(1.0 / self.resample_rate, 1.0, 1.0)) # slow pathway: 下采样 [bs, 1, t_slow, h, w] -> [bs, 1, t/8, h, w]
        x_slow = self.slow_path.conv1(x_slow) # [bs, 64, t_slow, h/2, w/2]
        x_slow = self.slow_path.maxpool(x_slow) # [bs, 64, t_slow, h/4, w/4]

        x_fast = nn.functional.interpolate(
            x,
            mode='nearest',
            scale_factor=(1.0 / (self.resample_rate // self.speed_ratio), 1.0,
                          1.0)) # fast pathway: 下采样 [bs, 1, t_fast, h, w] -> [bs, 1, t, h, w]
        x_fast = self.fast_path.conv1(x_fast) # [bs, 8, t_fast, h/2, w/2]
        x_fast = self.fast_path.maxpool(x_fast) # [bs, 8, t_fast, h/4, w/4]

        if self.slow_path.lateral: # True
            x_fast_lateral = self.slow_path.conv1_lateral(x_fast) # align the temporal dimension to slow pathway, map the channels from 8 to 16
            x_slow = torch.cat((x_slow, x_fast_lateral), dim=1) # [bs, 64+16, t_slow, h/4, w/4]

        for i, layer_name in enumerate(self.slow_path.res_layers):
            res_layer = getattr(self.slow_path, layer_name)
            x_slow = res_layer(x_slow) # after layer 1: [bs, 256, t_slow, h/4, w/4]
                                       # after layer 2: [bs, 512, t_slow, h/8, w/8]
                                       # after layer 3: [bs, 1024, t_slow, h/16, w/16]
                                       # after layer 4: [bs, 2048, t_slow, h/32, w/32]
            res_layer_fast = getattr(self.fast_path, layer_name) 
            x_fast = res_layer_fast(x_fast) # after layer 1: [bs, 32, t_fast, h/4, w/4]
                                            # after layer 2: [bs, 64, t_fast, h/8, w/8]
                                            # after layer 3: [bs, 128, t_fast, h/16, w/16]
                                            # after layer 4: [bs, 256, t_fast, h/32, w/32]
            if (i != len(self.slow_path.res_layers) - 1
                    and self.slow_path.lateral):
                # No fusion needed in the final stage
                lateral_name = self.slow_path.lateral_connections[i]
                conv_lateral = getattr(self.slow_path, lateral_name) # align the temporal dimension to slow pathway.
                                                                     # layer 1. map the channels from 32 to 64
                                                                     # layer 2. map the channels from 64 to 128
                                                                     # layer 3. map the channels from 128 to 256
