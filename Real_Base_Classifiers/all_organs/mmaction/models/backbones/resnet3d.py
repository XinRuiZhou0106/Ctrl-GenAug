...
class ResNet3d(nn.Module):
    ...
    def forward(self, x):
        ...
        """
        x = self.conv1(x) # [bs, 64, t/2, h/2, w/2]
        if self.with_pool1:
            x = self.maxpool(x) # [bs, 64, t/4, h/4, w/4]
        outs = []
        for i, layer_name in enumerate(self.res_layers):
            res_layer = getattr(self, layer_name)
            x = res_layer(x) # after layer 1: [bs, 256, t/4, h/4, w/4]
                             # after layer 2: [bs, 512, t/8, h/8, w/8] 
                             # after layer 3: [bs, 1024, t/8, h/16, w/16] 
                             # after layer 4: [bs, 2048, t/8, h/32, w/32]
            if i == 0 and self.with_pool2:
                x = self.pool2(x) # after layer 1 & pool: [bs, 256, t/8, h/4, w/4]
            if i in self.out_indices:
                # outs.append(nn.functional.interpolate(x, (8, 8, 8)))
                outs.append(x)
        if len(outs) == 1:
            return outs[0]
