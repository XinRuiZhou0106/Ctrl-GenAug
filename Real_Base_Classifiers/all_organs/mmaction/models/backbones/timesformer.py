...
class TimeSformer(nn.Module):
    ...
    def forward(self, x):
        ...
        cls_tokens = self.cls_token.expand(x.size(0), -1, -1) # 每一帧都有对应的cls token
        ...
        if self.attention_type != 'space_only':
            ...
            cls_tokens = x[:batches, 0, :].unsqueeze(1) # 可理解成video cls token
            x = rearrange(x[:, 1:, :], '(b t) p m -> (b p) t m', b=batches)
            x = x + self.time_embed
            x = self.drop_after_time(x)
            x = rearrange(x, '(b p) t m -> b (p t) m', b=batches)
            x = torch.cat((cls_tokens, x), dim=1) # [bs, 256*8+1, 768]
