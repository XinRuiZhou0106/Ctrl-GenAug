...
class C3D(nn.Module):
    ...
    def __init__(self,
        ...
        self.pool5 = nn.MaxPool3d(
            kernel_size=(1, 2, 2), stride=(2, 2, 2), padding=(0, 1, 1)) # original kernel size is (2, 2, 2)
    ...
    def forward(self, x):
        ...
        Args:
            x (torch.Tensor): The input data.
                the size of x is (num_batches, 1, 16, 112, 112).

        Returns:
            torch.Tensor: The feature of the input
            samples extracted by the backbone.
        """
        x = self.conv1a(x) # bs, 64, 16, 256, 256
        x = self.pool1(x) # bs, 64, 16, 128, 128

        x = self.conv2a(x) # bs, 128, 16, 128, 128
        x = self.pool2(x) # bs, 128, 8, 64, 64

        x = self.conv3a(x) # bs, 256, 8, 64, 64
        x = self.conv3b(x) # bs, 256, 8, 64, 64
        x = self.pool3(x) # bs, 256, 4, 32, 32

        x = self.conv4a(x) # bs, 512, 4, 32, 32
        x = self.conv4b(x) # bs, 512, 4, 32, 32
        x = self.pool4(x) # bs, 512, 2, 16, 16

        x = self.conv5a(x) # bs, 512, 2, 16, 16
        x = self.conv5b(x) # bs, 512, 2, 16, 16
        x = self.pool5(x) # bs, 512, 1, 9, 9

        x = x.flatten(start_dim=1)
        x = self.relu(self.fc6(x))
        x = self.dropout(x)
        x = self.relu(self.fc7(x)) # bs, 4096

        return x

if __name__ == '__main__':
    import torch
    model = nn.Conv3d(in_channels=1, out_channels=64, kernel_size=(3,3,3), padding=(1, 1, 1), stride=(1,1,1))
    input = torch.zeros((2, 3, 16, 112, 112))
